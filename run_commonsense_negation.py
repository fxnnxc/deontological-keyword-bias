...
def query_model_batch(batch_requests, max_new_tokens):
    """
...
def evaluate_dataset(dataloader, output_path, max_new_tokens):
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        for input_type, sentences in [("strong", strong_sentences), ("weak", weak_sentences), ("none", none_sentences)]:
            batch_results, batch_results_raw = query_model_batch(sentences, max_new_tokens=max_new_tokens)
...
if __name__ == "__main__":
    ...
    output_path = f"results/commonsense_negation/{question_format}_{answer_format}"
    os.makedirs(output_path, exist_ok=True)

    df = pd.read_csv("dataset/commonsense_negation.csv")
