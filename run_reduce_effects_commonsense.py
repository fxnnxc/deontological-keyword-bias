...
def query_model_batch(batch_requests, prompt_version, max_new_tokens):
    """
    ...
    if prompt_version == 'reasoning':
        ...
        flat_deontic_examples = [
            ("Employees must follow safety protocols in the laboratory.", 1),
            ("Parents have to register their children for kindergarten.", 1),
        ]
        flat_commonsense_examples = [
            ("You should bring flowers to a dinner party.", 0),
            ("I have to watch this movie tonight!", 0),
        ]

        def get_prompt_with_fewshot(version, batch_requests):
            import re
            ...
            if match:
                ...
                instruction = (
                    ...
                    "Focus on modal verbs (must, should, have to, ought to) and evaluate if they are appropriately used to express deontic meaning.\n"
                    "You must answer with ONLY 1 or 0. 1 = deontic, 0 = not deontic.\n\n"
                )

                few_shots = ""
                for sentence, label in selected_deontic + selected_common:
                    few_shots += f"Input: {sentence}\nAnswer: {label}\n\n"

                prompts = [
                    instruction + few_shots + f"Input: {item}\nAnswer: "
                    for item in batch_requests
                ]
                return prompts
        ...
        prompts = get_prompt_with_fewshot(prompt_version, batch_requests)
        ...
        def get_prompt_with_logicshot(version, batch_requests):
            ...
            if match:
                ...
                instruction = (
                    ...
                    "Logic Example:\n"
                    "Input: Citizens are required by law to pay taxes.\n"
                    "Output: Although the word 'must' is not used, the phrase 'required by law' clearly indicates a legal obligation. This expresses a deontic rule.\n"
                    "Answer: 1\n\n"
                    "Input: {item}\nOutput:"
                )
                prompts = [
                    instruction_prev +few_shots + instruction.format(item=item)
                    for item in batch_requests
                ]
                return prompts
            return None
...
def evaluate_dataset(dataloader, output_path, prompt_version, max_new_tokens):
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        for input_type, sentences in [("strong", strong_sentences)]:
            ...
            batch_results, batch_results_raw = query_model_batch(sentences, prompt_version, max_new_tokens=max_new_tokens)
            for inp, res, res_raw in zip(sentences, batch_results, batch_results_raw):
                results.append({
...
if __name__ == "__main__":
    ...
    output_path = f"results/reduce_effects_commonsense/{args.prompt_version}"
    os.makedirs(output_path, exist_ok=True)

    df = pd.read_csv("dataset/reduce_effects_commonsense.csv")
